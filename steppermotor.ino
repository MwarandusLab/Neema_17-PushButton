...
void loop() {
  ...
  if(digitalRead(ledPin) == HIGH){
    int steps = (90 * stepsPerRevolution) / 360;
      ...
      // Rotate the stepper motor 90 degrees in one direction(anticlockwise)
    stepper.move(-steps);
    ...
    int steps = (180 * stepsPerRevolution) / 360;

      // Rotate the stepper motor 180 degrees in one direction(clockwise)
    stepper.move(steps);
